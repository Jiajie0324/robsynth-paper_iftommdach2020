...
As reasoned in \cite{JamwalHusXie2015,Ramirez2018}, performing a multi-objective optimization requires to select a solution on the pareto-front in the hyper-surface of all different objective criteria.
...
\end{equation}
%
where $z$ represents the objective function as opposed to the fitness function $f$, which contains the penalty terms in a piece-wise definition, which is depicted in Fig.\,\ref{fig:uebersicht}.

To increase the convergence of the optimization with respect to the constant penalties of \cite{Ramirez2018}, the degree of violation $\rho>0$ of the constraints is included in the penalties $v_i$ and normalized with the function
%
\begin{equation}
...
\begin{itemize}
\item base diameter $d_\mathrm{base}=p_\mathrm{scale} p_\mathrm{base}$
\item platform diameter $d_\mathrm{plf}=p_\mathrm{plfscale} d_\mathrm{base}$
\item distance $d_\mathrm{pairdist}=d_\mathrm{base} p_\mathrm{pdscale}$ of coupling point pairs, in case of a pair-wise alignment at the base (Fig.\,xxx z)
\item pair distance xxx for the moving platform
\item elevation of the coupling joint, in case of a pyramidal alignment of the first joint axis of the leg chains
\end{itemize}


* Definition von Parametern: Abhängigkeit zwischen den Parametern, Skalierung
