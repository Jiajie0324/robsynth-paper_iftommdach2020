...
The structural synthesis of parallel robots by means of screw theory \cite{KongGos2007} or evolutionary morphology \cite{Gogu2008} has produced a variety of different parallel robots.
...
An automatic dimensioning (dimensional synthesis) for all existing types of robots is necessary to select the robot that is suited best for a specific given application.
It is well established that the theoretical advantages of parallel robots are strongly dependent on the dimensioning and the dimensional synthesis has a more important role than for serial robots \cite{Merlet2006}.
...
Efforts on the dimensional synthesis constitute a major part in the research of parallel robots, beginning in the early 2000s driven by international robotics research groups \cite{CarboneOttCec2007,KelaiaiaComZaa2012,JamwalHusXie2015} and German researchers in the area of machine tools \cite{MbarekNefCor2005,Krefft2006,FrindtKreHes2010}.
...
This has advantages for the stiffness of the machine, which is essential for machining tasks \cite{MbarekNefCor2005}.
...
Only few authors report a dimensional synthesis for multiple different robots with the ambition to compare them by this means, e.g. \cite{Krefft2006} with two different architectures of Delta robots or \cite{Ramirez2018} with serial robots.
...
The synthesis is mainly focused on kinematic characteristics such as workspace and conditioning of the Jacobian \cite{CarboneOttCec2007,KelaiaiaComZaa2012} or kinetostatic characteristics such as the stiffness \cite{CarboneOttCec2007,KelaiaiaComZaa2012}.% of the manipulator and its isotropy in the workspace and in different directions [].
...
In the dimensional synthesis of serial robots, the dynamics gets more attention, e.g. by using a multi-body simulation tool like \textsc{Adams} in the optimization loop \cite{ZhouBaiHan2011}.
...
A consideration of material stress and internal forces is to the best knowledge of the authors only performed in the dimensional synthesis of serial robots, e.g. by \cite{ZhouBai2015} who perform FE analyses with Ansys for a parametric CAD model inside the optimization loop.

Usually, a heuristic multi-objective optimization is performed in the dimensional synthesis, e.g. with the Strength Pareto Evolutionary Algorithm \cite{Krefft2006,KelaiaiaComZaa2012} or the Nondominated Sorting
Algorithm (NSGA II) \cite{JamwalHusXie2015}.
The single-objective optimization is claimed to be insufficient by \cite{JamwalHusXie2015}.
...
\item the application of a general kinematics and dynamics model on the dimensional synthesis of non-conventional parallel robots,
\item the proposition of an efficient structure for the optimization of robot dimensions
\item examples and simulative results for the proposed approach.
\end{compactitem}
...
%
can be computed with the recursive Newton-Euler algorithm for serial link kinematics \cite{KhalilDom2002}.
