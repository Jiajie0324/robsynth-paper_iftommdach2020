...
Performing a multi-objective optimization requires to select a solution on the pareto-front in the hyper-space of all different objective criteria.
When trying to select the best robot architecture from many possibilities for a given task, an unambiguous criterion has to be used not necessarily requiring a human decision.
...
The penalty terms $v_i$ are defined hierarchically according to the infeasibility of the solution with
%
\begin{equation}
...
To increase the convergence of the optimization, the degree of violation $\rho>0$ of the constraints is included in the penalties and normalized with the function
%
\begin{equation}
f_\mathrm{norm}(\rho) = \frac{2}{\pi} \mathrm{arctan}(\rho)
%\quad \mathrm{with} \quad
...
The range of values for the energy taken from the grid from (\ref{equ:energy_integral}) is not known in advance.
...
\end{equation}
%
is therefore constructed by normalizing the energy value.
