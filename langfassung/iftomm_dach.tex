...
\begin{figure*}[b]
    ...
    \captionof{figure}{Ergebnisse der Maßsynthese für vier Beispiel-PKM mit Angabe der Zielfunktion $E_\mathrm{grid}$ und der Parameteranzahl $\mathrm{dim}(\bm{p})$.}
...
The functionality of the hierarchic constraints and the convergence of the optimization is verified in Sec.\,\ref{sec:res_convergence}.
An analysis on the use of the material stress constraint is performed in Sec.\,\ref{sec:res_matstress} and the influence of tilting angles on the overall performance is investigated in Sec.\,\ref{sec:res_condition_pivoting}.
...
The architectures for the dimensional synthesis are the 6\underline{P}RRS- and 6\underline{R}RRS PKM, shown in Fig.\,\ref{fig:ergebnisse},\,a,\,c, where ``6'' marks the number of leg chains, ``R'' denotes a revolute, ``P'' a prismatic and ``S'' a spherical joint and the underscore marks the actuation.
The generality of the kinematics and dynamics model laid out in Sec.\,\ref{sec:sysmdl} is highlighted by comparing two different variants of the architectures: One robot with leg chains ending in spherical joints (S), Fig.\,\ref{fig:ergebnisse},\,a,\,c, and one with leg chains ending in three distinctive revolute joints (RRR), Fig.\,\ref{fig:ergebnisse},\,b,\,d.
...
The kinematics of the leg chains are the result of a structural synthesis \cite{Ramirez2018} and are part of a minimal set of serial link kinematic chains.
This results in a very general description of the kinematics with many free parameters, with only the parameters
%
\begin{align}
\alpha_{1,\mathrm{DH}}=0, \quad \alpha_{5,\mathrm{DH}}=\pi/2, \quad \alpha_{6,\mathrm{DH}}=\pi/2
\end{align}
%
(in the modified DH-notation of Khalil \cite{KhalilDom2002}) set constant for all robots.
For the architecture I (6\underline{P}RRS), only the DH-Parameters $a$/$d$ of the last two joints are set to zeros due to the S-joint.
%
...
%
For architecture III (6\underline{R}RRS), a simplified base and platform coupling of the leg chains is selected, leaving only 12 parameters, including the scaling parameter, and
...
The parallel robots have to execute a reference trajectory in the workspace center, which is a cube with edge length 30\,mm, visible in Fig.\,\ref{fig:ergebnisse}.
The tilting angles of the end effector are chosen as up to $90^\circ$, depending on the evaluation in the following sections.
To make demands on the robot dynamics, which is the basis for the optimization criterion energy, the corners are connected with rest-to-rest motions with an acceleration trapezoid profile.
%\begin{equation}
...
Further work will extend the constraint formulation with considering self-collisions and comparing robots with and without task-redundancy.
